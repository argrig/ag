...
class User {
  function login($parts){
    ...
    strip($_POST['login'],$_POST['passwd']) ;
    $values = array($_POST['login'],$_POST['login']) ;
    $query = <<<EOF
      SELECT u.name name, surname, login, passwd, email, u.verif verif, created, lastlog, unvid, roleid,
...
EOF;
    ...
    }else if(password_verify($_POST['passwd'],$user['passwd'])){
      unset($user['passwd']) ;
      foreach($user as $key=>$val) {$_SESSION[$key] = $val ;}
      foreach(array('unvid','roleid','rname','unvverif') as $key) {unset($user[$key]);}
      sendJSON($user) ; 
    }else {
